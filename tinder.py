...
class Tinder:
    ...
    def loginWindow(self):
        self._root=Tk()
        self._root.configure(background="#FF6E55")
        self._root.minsize(400,600)

        self._label1=Label(self._root, text="Tinder", bg="#FF6E55", fg="#fff")
        ...
        self._regBtn = Button(self._root, text="Click Here", bg="#fff", width=10, height=1,command=lambda: self.regWindow())
        self._regBtn.pack()
    ...
    def regWindow(self):
        ...
        self._registerBtn = Button(self._root1, text="Register", bg="#fff", width=30, height=2, command=lambda: self.regHandler(self._nameInput.get(), self._emailInput.get(), self._passwordInput.get(), self._ageInput.get(),self._genderInput.get(),self._cityInput.get(),self._bioInput.get()))
    ...
    def othersProfile(self, num):
        send_data=[]
        data=self._dbo.searchOne('user_id',self.user_id,'users','NOT LIKE')

        if num<0:
            self.errorMessage("Error","Kahan ja raha hai be")
        elif num>len(data)-1:
            self.errorMessage("Error","Khatm ho gaya bhai")
        else:
    ...
    def propose(self,juliet_id):
        ...
        if len(data)==0:
            ...
            if flag==1:
                self.errorMessage("Success","Bhej diya hai.Fingers crossed")
            else:
                self.errorMessage("Failure","Beda gark naha ke aana")
        else:
